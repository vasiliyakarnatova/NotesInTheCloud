...
const Home = () => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        const response = await fetch("http://localhost:8081/api/NotesInTheCloud/current-user", { // fetch current user
  ...
  const handleLogout = async () => {
    try {
      await fetch("http://localhost:8081/api/NotesInTheCloud/logout", {
