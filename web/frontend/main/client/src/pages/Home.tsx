...
export default function Home() {
  ...
  const updateTodoMutation = useMutation({
    mutationFn: async ({ id, noteId, completed }: { id: string; noteId: string; completed: boolean }) => {
      const res = await apiRequest("PUT", `/api/notes/${noteId}/tasks/${id}/checkbox`, { completed });
      return res.json();
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ["/api/notes", variables.noteId] });
  ...
  const editTodoMutation = useMutation({
    mutationFn: async ({ id, noteId, text }: { id: string; noteId: string; text: string }) => {
      const res = await apiRequest("PUT", `/api/notes/${noteId}/tasks/${id}/edit`, { text });
      return res.json();
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ["/api/notes", variables.noteId] });
  ...
  const handleEditTodo = (id: number | string, noteId: string, text: string) => {
    if (typeof id === 'string') {
      editTodoMutation.mutate({ id, noteId, text });
    } else {
      console.log("Editing temporary todo:", id, text);
    }
  };
  ...
  const deleteTodoMutation = useMutation({
    mutationFn: async ({ id, noteId }: { id: string; noteId: string }) => {
      await apiRequest("DELETE", `/api/todos/${id}`, { noteId });
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ["/api/notes", variables.noteId] });
      toast({
  ...
  const handleUpdateTodo = (id: number | string, noteId: string | number, completed: boolean) => {
  ...
  const handleDeleteTodo = (id: number | string, noteId: string | number) => {
    if (typeof id === 'string' && typeof noteId === 'string') {
          ...
          <>
            {activeNoteId && activeNote ? (
              <NoteViewer
                ...
                onEditTodo={(todoId, text) => handleEditTodo(todoId, activeNote.id, text)}
